...
content-editor の current release-driven development で使用する Operating Model を定義します。
...
- SQLite content files / schema / API specification の既存 documentation を project-specific authority として維持する。
- current pnpm-based project commands are preserved; adopting mise or changing package manager requires an explicit toolchain decision rather than this governance update.
- user/content databases are mutable product data and must not be treated as disposable agent workspace state.

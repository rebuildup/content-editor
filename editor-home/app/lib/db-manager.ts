...
import type { DatabaseInfo, DatabaseStats } from '@/types/database';
...
export function createDatabase(id: string, name: string, description?: string): boolean {
    const config = loadConfig();
...
export function copyDatabase(sourceId: string, targetId: string, name: string, description?: string): boolean {
    const config = loadConfig();
...
export function getDatabaseStats(dbId: string): DatabaseStats | null {
    ...
    try {
        ...
        const markdownPagesCount = db.prepare('SELECT COUNT(*) as count FROM markdown_pages').get() as { count: number };
        const tagsCount = db.prepare('SELECT COUNT(DISTINCT tag) as count FROM content_tags').get() as { count: number };
...
export function updateDatabaseInfo(dbId: string, name: string, description?: string): boolean {
    const config = loadConfig();

...
import type { MarkdownPage, MarkdownFrontmatter, MarkdownFile, MarkdownStats } from "@/types/markdown";
 ...
 */
export function stringifyFrontmatter(frontmatter: MarkdownFrontmatter, body: string): string {
    const lines = ['---'];
...
export function calculateMarkdownStats(body: string): MarkdownStats {
    ...
    // 単語数（日本語対応）
    const words = body.match(/[\w\u4e00-\u9faf\u3040-\u309f\u30a0-\u30ff]+/g) || [];
...
export function markdownPageToRow(page: Partial<MarkdownPage>): Partial<MarkdownPageRow> {
    return {
        id: page.id,
...
// ========== Markdownページ取得 ==========
export function getMarkdownPage(db: Database.Database, idOrSlug: string): MarkdownPage | null {
    // IDまたはスラッグで検索
    let row = db.prepare('SELECT * FROM markdown_pages WHERE id = ?').get(idOrSlug) as MarkdownPageRow | undefined;

    if (!row) {
        row = db.prepare('SELECT * FROM markdown_pages WHERE slug = ?').get(idOrSlug) as MarkdownPageRow | undefined;
    }
...
// ========== Markdownページ保存 ==========
export function saveMarkdownPage(db: Database.Database, page: Partial<MarkdownPage>): void {
...
// ========== Markdownページ削除 ==========
export function deleteMarkdownPage(db: Database.Database, idOrSlug: string): boolean {
    const result = db.prepare('DELETE FROM markdown_pages WHERE id = ? OR slug = ?').run(idOrSlug, idOrSlug);
...
export function importMarkdownFile(file: MarkdownFile): MarkdownPage {
    ...
    const slug = frontmatter.slug || file.filename.replace(/\.md$/, '').toLowerCase()
        .replace(/[^a-z0-9\-_]/g, '-')
        .replace(/-+/g, '-')
        .replace(/^-|-$/g, '');

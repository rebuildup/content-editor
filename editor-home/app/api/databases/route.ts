...
export async function POST(req: Request) {
    try {
        ...
        if (data.action === 'create') {
            ...
            if (!success) {
                return Response.json({ error: 'Database already exists or invalid ID' }, { status: 400 });
            }
        ...
        // データベースコピー
        if (data.action === 'copy') {
            const success = copyDatabase(data.sourceId, data.targetId, data.name, data.description);
            if (!success) {
                return Response.json({ error: 'Failed to copy database' }, { status: 400 });
            }
...
export async function DELETE(req: Request) {
    try {
        ...
        if (!success) {
            return Response.json({ error: 'Cannot delete active database or database not found' }, { status: 400 });
        }
